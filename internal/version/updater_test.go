...
func TestReleaseAssetNameAndDownloadURL(t *testing.T) {
	...
	source := ReleaseSource{
		Name:            "ghproxy.net",
		DownloadBaseURL: "https://ghproxy.net/https://github.com/HackenLeung/ccLoad/releases/download",
	}
	assetURL, err := releaseDownloadURL(source, "v2.44.0", name)
	if err != nil {
		t.Fatalf("releaseDownloadURL(asset): %v", err)
	}
	if assetURL != "https://ghproxy.net/https://github.com/HackenLeung/ccLoad/releases/download/v2.44.0/ccload-linux-amd64" {
	...
	if checksumURL != "https://ghproxy.net/https://github.com/HackenLeung/ccLoad/releases/download/v2.44.0/checksums.txt" {
...
func TestAutoUpdaterReleaseSourceConfiguration(t *testing.T) {
	...
	t.Run("defaults prefer ghproxy then GitHub", func(t *testing.T) {
		...
		assertAutoUpdaterLatestRequests(t, []string{
			"https://ghproxy.net/https://github.com/HackenLeung/ccLoad/releases/latest",
			"https://github.com/HackenLeung/ccLoad/releases/latest",
		})
	})

	t.Run("custom base disables built-in fallback", func(t *testing.T) {
		t.Setenv("CCLOAD_RELEASE_BASE_URL", "https://mirror.example/https://github.com/HackenLeung/ccLoad/releases/latest/download/")
		assertAutoUpdaterLatestRequests(t, []string{
			"https://mirror.example/https://github.com/HackenLeung/ccLoad/releases/latest",
		})
	})
...
func TestUpdateOnceReplacesPendingVersionWithNewerDownloadedRelease(t *testing.T) {
	...
	updater, err := NewAutoUpdater(AutoUpdateOptions{
		...
		ReleaseSources: []ReleaseSource{{
			...
			DownloadBaseURL: server.URL + "/HackenLeung/ccLoad/releases/download",
		}},
