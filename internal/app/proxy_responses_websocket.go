...
func readResponsesWebsocketMessages(
	...
	// Keep reading while an upstream turn is running so control frames and the
	// next client message do not block the WebSocket reader.
	messages := make(chan responsesWebsocketInboundMessage, 8)
	go func() {
		defer cancel()
		for {
...
func (w *responsesWebsocketBridgeWriter) Write(data []byte) (int, error) {
	...
	for {
		...
		var err error
		payload, err = addResponsesWebsocketStreamID(payload, w.streamID)
		if err != nil {
			return 0, err
		}
...
func addResponsesWebsocketStreamID(payload []byte, streamID string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("attach websocket stream_id: %w", err)
	}
