...
func (s *Server) handleChannelTestRequest(c *gin.Context, requireBaseURL bool) {
	...
	s.persistDetectionLog(c.Request.Context(), detectionLogFromResult(cfg, model.LogSourceManualTest, requestedModel, testReq.Model, keySelection.apiKey, c.Request.Header, 0, testReq.ThinkingEffort, testResult))
...
func (s *Server) buildTestUpstreamRequest(
	...
	// 与 requestBody 的协议一致。
	requestPlan.requestBody = normalizeChannelDeveloperRole(cfg, extractRequestPath(requestPlan.fullURL), requestPlan.requestBody)
