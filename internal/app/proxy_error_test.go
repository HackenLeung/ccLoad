	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				...
				_, action = srv.handleNetworkError(ctx, cfg, 0, "test-model", "test-key", 0.1, err, nil, reqCtx, false)
			} else {
...
func Test_HandleNetworkError_Basic(t *testing.T) {
	...
	t.Run("context canceled returns client error", func(t *testing.T) {
		result, action := srv.handleNetworkError(
			ctx, cfg, 0, "test-model", "test-key", 0.1, context.Canceled, nil, reqCtx, false,
		)
	...
	t.Run("network error switches channel", func(t *testing.T) {
		result, action := srv.handleNetworkError(
			ctx, cfg, 0, "test-model", "test-key", 0.1, errors.New("connection refused"), nil, reqCtx, false,
		)
	...
	t.Run("first byte timeout switches channel", func(t *testing.T) {
		...
		result, action := srv.handleNetworkError(
			ctx, cfg, 0, "test-model", "test-key", 0.1, err, nil, reqCtx, false,
		)
...
func Test_HandleProxyError_499(t *testing.T) {
	...
	t.Run("client canceled returns to client", func(t *testing.T) {
		...
		_, action := srv.handleNetworkError(ctx, cfg, 0, "test-model", "test-key", 0.1, context.Canceled, nil, reqCtx, false)
...
func Test_HandleNetworkError_499_PreservesTokenStats(t *testing.T) {
	...
	result, action := srv.handleNetworkError(
		ctx, cfg, 0, "claude-sonnet-4-5", "test-key", 0.5, context.Canceled, res, reqCtx, false,
	)
