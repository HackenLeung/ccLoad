...
func cleanupMySQLTables(t *testing.T, db *sql.DB) {
	...
	tables := []string{"logs", "web_sessions", "admin_sessions", "system_settings", "auth_tokens", "global_disabled_models", "channel_model_aliases", "channel_models", "api_keys", "channels", "schema_migrations"}
...
func TestMySQL(t *testing.T) {
	...
	t.Run("FullMigration", func(t *testing.T) {
		...
		// 验证关键表存在
		tables := []string{"channels", "api_keys", "channel_models", "channel_model_aliases", "global_disabled_models", "auth_tokens", "logs", "system_settings", "web_sessions"}
		for _, table := range tables {
			var count int
