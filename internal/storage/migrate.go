...
func migrate(ctx context.Context, db *sql.DB, dialect Dialect) error {
	...
	for _, defineTable := range tables {
		...
		if tb.Name() == "channels" {
			...
			if err := ensureColumn(ctx, db, dialect, "channels", "responses_transport", "VARCHAR(32) NOT NULL DEFAULT 'http'", "TEXT NOT NULL DEFAULT 'http'"); err != nil {
				return fmt.Errorf("migrate channels responses_transport: %w", err)
			}
...
func initDefaultSettings(ctx context.Context, db *sql.DB, dialect Dialect) error {
	settings := []struct {
		...
		{"channel_check_interval_hours", "0", "float", "渠道定时检测间隔(小时,支持小数如0.5=30分钟,0=关闭,修改后重启生效)", "0"},
		...
		{"cooldown_all_cooled_wait_seconds", "60", "int", "所有渠道都在冷却时，最多等待多久待渠道恢复后重试一轮(秒,0=不等待直接返回错误)", "60"},
		// Debug日志配置
	...
	// 迁移 channel_check_interval_hours 类型：int → float（支持分钟级小数间隔）
	{
		...
		//nolint:gosec // G201: keyCol 仅为 "key" 或 "`key`"，由内部逻辑控制
		typeSQL := fmt.Sprintf("UPDATE system_settings SET value_type = 'float', description = '渠道定时检测间隔(小时,支持小数如0.5=30分钟,0=关闭,修改后重启生效)', default_value = '0' WHERE %s = 'channel_check_interval_hours' AND value_type = 'int'", keyCol)
		if _, err := db.ExecContext(ctx, typeSQL); err != nil {
