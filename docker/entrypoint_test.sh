...
test_default_falls_back_to_github() {
  ...
  case "$first_url" in
    https://ghproxy.net/https://github.com/HackenLeung/ccLoad/releases/latest/download/*) ;;
    *) fail "first default request did not use ghproxy: $first_url" ;;
  esac
  grep -q '^https://github.com/HackenLeung/ccLoad/releases/latest/download/' "$case_dir/curl.log" ||
...
test_custom_source_does_not_fallback() {
  ...
  run_entrypoint "$case_dir" "https://mirror.example/HackenLeung/ccLoad/releases/latest/download"
...
test_invalid_custom_source_fails_fast() {
  ...
  if run_entrypoint "$case_dir" "https://mirror.example/HackenLeung/ccLoad/releases/download"; then
