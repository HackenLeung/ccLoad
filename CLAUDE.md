...
- **developer 角色降级**:`messages[].role=developer`(新版 OpenAI SDK 对 system 的更名)统一改写为 `system`——多数上游 role 枚举只有 system/user/assistant/tool,原样发出会 400 且换 Key/换渠道都救不回来。两处无条件生效:编码端 `request_prompt_openai.go:openAIWireRole`(覆盖本地转换)、直通端 `proxy_util.go:normalizeDeveloperMessageRole`(仅 chat/completions 与 messages 族;Codex `/v1/responses` 的 `input[]` 里 developer 合法,不碰)。改写走 `json.RawMessage` 只动 role 字段保精度,并用 `stableSonicCfg`(SortMapKeys)保证字节稳定,否则 map 随机迭代顺序会打掉上游 prompt cache 命中
